 ...
 * 
 */
public class MessagingAdapter extends ProxyFactoryBean implements InitializingBean, DisposableBean, MethodInterceptor,
		MessageHandler<InvocationWrapper> {

	private final static Logger log = LoggerFactory.getLogger(MessagingAdapter.class);

	/* common config */
	private Messenger.Factory factory; // required. inject for one connection
										// per-app
	...
	private boolean isSyncRun; // default false. depend on service method return
								// type and destination definition
	...
	public Object invoke(MethodInvocation mi) throws Throwable {
		...
		InvocationWrapper msg = new InvocationWrapper(className, mi);
	...
	public void onMessage(final InvocationWrapper invoker) {
		if (!invoker.getClassName().equals(this.getTargetClass().getName())) {
			return;
		}
	...
	private void invoke(InvocationWrapper invoker) {
		try {
			...
			log.error("Service invoke error. targetClass=" + this.getTargetClass() + ", methodName"
					+ invoker.getMethodName(), e);
		}
	}
...
class InvocationWrapper implements Externalizable {
	...
	public InvocationWrapper(String className, MethodInvocation mi) {
