...
import incubation.app.component.ComponentConfig;
import incubation.app.component.ComponentException;
import incubation.app.component.ComponentLoaderConfig;
import incubation.app.start.Classpath;
		...
		if (!parentPath.exists() || !parentPath.isDirectory()) {
			...
			if (componentLoadConfig != null && componentLoadConfig.exists()) {
				...
				try {
					...
					List<ComponentLoaderConfig.ComponentDef> componentsToLoad = ComponentLoaderConfig
							.getComponentsFromConfig(configUrl);
					...
					log.error("Unable to locate URL for component loading file: "
							+ componentLoadConfig.getAbsolutePath(), e);
				} catch (Exception e) {
						...
						if (componentPath.isDirectory()) {
							...
							if (configFile.exists()) {
								...
								try {
									...
									log.error("Cannot load component: " + componentPath.getName() + "@"
											+ componentLocation, e);
								}
								if (config == null) {
									log.error("Cannot load component: {}@{}. {}", componentPath.getName(),
