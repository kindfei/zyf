package samples.jmx.lookup.ldap;
...
/**
 * This class demonstrates how to use an LDAP directory as a lookup service for
 * JSR 160 connectors. It shows how to register a JMXConnectorServer with the
 * LDAP directory through JNDI.
 * <p>
 * See README file and {@link #main(String[])} for more details.
 * <p>
 * Make sure to read the section "Binding with Lookup Services" of the JMX
 * Remote API 1.0 Specification before looking at this example.
 */
public class Server {
	...
	/**
	 * Get a pointer to the root context of the directory tree under which this
	 * server is supposed to register itself. All LDAP DNs will be considered to
	 * be relative to that root.
	 * <p>
	 * Note that this root is not part of the JSR 160 specification, since the
	 * actual location where a JMX Agent will register its connectors is left
	 * completely open by the specification. The specification only discuss what
	 * the JMX Agent must/may put in the directory - but not where.
	 * <p>
	 * This method assumes that the root of the directory is will be passed in a
	 * the {@link Context#PROVIDER_URL Context.PROVIDER_URL} System property.
	 * <p>
	 * This method will transfer a fixed set of System Properties to the
	 * Hashtable given to the JNDI InitialContext:
	 * <ul>
	 * <li>{@link Context#INITIAL_CONTEXT_FACTORY 
	 * Context.INITIAL_CONTEXT_FACTORY} - default is
	 * <code>"com.sun.jndi.ldap.LdapCtxFactory"</code></li>
	 * <li>{@link Context#PROVIDER_URL  Context.PROVIDER_URL}</li>
	 * <li>{@link Context#SECURITY_PRINCIPAL  Context.SECURITY_PRINCIPAL} -
	 * default is <code>"cn=Directory Manager"</code></li>
	 * <li>{@link Context#SECURITY_CREDENTIALS Context.SECURITY_CREDENTIALS}</li>
	 * </ul>
	 * 
	 ...
	 * <p>
	 * This method expects to find the LDAP DN where it will register the JMX
	 * Connector URL in the "dn" System property. If that property is not set,
	 * then "cn=<var>name</var>" is assumed.
	 * <p>
	 * If the given DN does not point to an existing node in the directory, then
	 * this method will attempt to create it. Yet, the parent node must already
	 * exist in that case.
	 * <p>
	 * If the DN points to a node that is already of the <var>jmxConnector</var>
	 * class, then this method will simply override its <var>jmxServiceURL</var> ,<var>jmxAgentName</var>,
	 * <var>jmxProtocolType</var>, <var>jmxAgentHost</var> and
	 * <var>jmxExpirationDate</var> attributes.
	 * 
	 * @param root
	 *            A pointer to the root context we are using, as returned by
	 *            {@link #getRootContext()}.
	 * @param jmxUrl
	 *            A JMX Connector Server URL, that should have been obtained
	 *            from {@link JMXConnectorServer#getAddress()
	 *            JMXConnectorServer.getAddress()};
	 * @param name
	 *            The AgentName with which the URL must be registered in the
	 *            LDAP directory.
	 */
	...
	/**
	 * Creates an RMI Connector Server, starts it, and registers it with the
	 * LDAP directory.
	 * <p>
	 * This method will transfer a fixed set of System Properties to the Map
	 * given to the RMIConnectorServer constructor. Some JNDI properties, if
	 * defined, are transfered to the Map so that they may be used when LDAP is
	 * used as external directory to register the RMI Stub (see
	 * {@link javax.management.remote.rmi} Javadoc). Note that even if LDAP is
	 * used as external directory the {@link Context#INITIAL_CONTEXT_FACTORY 
	 * Context.INITIAL_CONTEXT_FACTORY} and
	 * {@link Context#PROVIDER_URL Context.PROVIDER_URL} properties usually
	 * don't need to be passed.
	 * <p>
	 * The following System properties, if defined, are transfered to the Map
	 * given to the RMIConnectorServer constructor.
	 * <ul>
	 * <li>{@link Context#INITIAL_CONTEXT_FACTORY 
	 * Context.INITIAL_CONTEXT_FACTORY}</li>
	 * <li>{@link Context#PROVIDER_URL  Context.PROVIDER_URL}</li>
	 * <li>{@link Context#SECURITY_PRINCIPAL  Context.SECURITY_PRINCIPAL}</li>
	 * <li>{@link Context#SECURITY_CREDENTIALS Context.SECURITY_CREDENTIALS}</li>
	 * <li>{@link RMIConnectorServer#JNDI_REBIND_ATTRIBUTE
	 * RMIConnectorServer.JNDI_REBIND_ATTRIBUTE} - default is <code>true</code>.</li>
	 * </ul>
	 * 
	 * @param url
	 *            A string representation of the JMXServiceURL.
	 * @return the created RMIConnectorServer.
	 */
	public JMXConnectorServer rmi(String url) throws IOException, JMException, NamingException, ClassNotFoundException {
	 ...
	 * 
	 * @param server
	 *            the JMXConnectorServer to start and register.
	 * @param env
	 *            the environment Map.
	 * @param agentName
	 *            the AgentName with which the URL must be registered in the
	 *            LDAP Directory. This is not a LDAP DN, but the value of the
	 *            jmxAgentName attribute.
	 */
	...
	public static String getExpirationDate(long sec) {
		...
		if (fmt.getCalendar().getTimeZone().inDaylightTime(date))
			gmtDate = new Date(System.currentTimeMillis() - fmt.getCalendar().getTimeZone().getRawOffset()
					- fmt.getCalendar().getTimeZone().getDSTSavings() + 1000 * sec);
		else
	 ...
	 * Program Main
	 * <p>
	 * Creates a server object, gets the JMX Service URL, and calls the method
	 * that will create and register the appropriate JMX Connector Server for
	 * that URL.
	 * <p>
	 ...
	 *     </code>:
	 * The initial context factory to use for accessing the LDAP directory (see
	 * {@link Context#INITIAL_CONTEXT_FACTORY Context.INITIAL_CONTEXT_FACTORY}) -
	 * default is <code>"com.sun.jndi.ldap.LdapCtxFactory"</code>.</li>
	 * <li><code>-Djava.naming.provider.url=&lt;provider-url&gt;</code>: The
	 * LDAP Provider URL (see {@link Context#PROVIDER_URL 
	 * Context.PROVIDER_URL}).</li>
	 * <li><code>-Djava.naming.security.principal=&lt;ldap-principal&gt;
	 *     </code>:
	 * The security principal (login) to use to connect with the LDAP directory
	 * (see {@link Context#SECURITY_PRINCIPAL  Context.SECURITY_PRINCIPAL} -
	 * default is <code>"cn=Directory Manager"</code>.</li>
	 * <li><code>-Djava.naming.security.credentials=&lt;ldap-credentials&gt;
	 *     </code>:
	 * The security credentials (password) to use to connect with the LDAP
	 * directory (see {@link Context#SECURITY_CREDENTIALS
