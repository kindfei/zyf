 ...
 * JSR 160 connectors. It shows how to lookup a JMXServiceURL from the LDAP
 * directory.
 * <p>
...
public class Client {
	 ...
	 *            The root DirContext.
	 * @param dn
	 *            The DN of the node, relative to the root DirContext.
	 */
	...
	/**
	 * Parses the expirationDate in order to determined whether the associated
	 * URL has expired.
	 * 
	 * @param expirationDate
	 *            an X.208 GeneralizedTime, local or GMT. Only yyyyMMddHHmmss.S
	 *            (local time) and yyyyMMddHHmmss.SZ (GMT time) formats are
	 *            recognized.
	 * @return true if the expirationDate could be parsed and is past, false
	 *         otherwise.
	 */
	...
	/**
	 * Returns a date in the local time zone parsed from an X.208 formatted
	 * date. Only yyyyMMddHHmmss.S (local time) and yyyyMMddHHmmss.SZ (GMT time)
	 * formats are recognized.
	 * 
	 * @param expirationDate
	 *            an X.208 GeneralizedTime, local or GMT.
	 ...
	 * 
	 * @param root
	 *            A pointer to the LDAP directory, returned by
	 *            {@link #getRootContext()}.
	 * @param protocolType
	 *            The protocol type of the JMX Connectors we want to retrieve.
	 *            If <var>protocolType</var> is null, then the jmxProtocolType
	 *            attribute is ignored. Otherwise, only those agents that have
	 *            registered a matching jmxProtocolType attribute will be
	 *            returned.
	 * @param name
	 *            the AgentName of the JMXConnectors that should be returned. If
	 *            <var>name</var> is null, then the JMXConnectors for all
	 *            agents are returned (null is an equivalent for a wildcard).
	 * @return The list of matching JMXConnectors retrieved from the LDAP
	 *         directory.
	 */
	public static List lookup(DirContext root, String protocolType, String name) throws IOException, NamingException {
		...
		//
		String query = "(&" + "(objectClass=jmxConnector) " + "(jmxServiceURL=*) " + queryProtocol + "(jmxAgentName="
				+ ((name != null) ? name : "*") + "))";
	 ...
	 * Program Main.
	 * <p>
	 * Lookup all JMX agents in the LDAP Directory and list their MBeans and
	 * attributes.
	 * <p>
	 ...
	 * client to lookup for a specific protocol type (default is null, meaning
	 * any type).</li>
